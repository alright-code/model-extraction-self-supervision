...
class ImageNetClassifier(pl.LightningModule):
    def __init__(self, finetune, data_fraction, pretrained_chkpt, temperature, logits_file, **kwargs):
        super().__init__()
    ...
    def configure_optimizers(self):
        ...
        if self.finetune:
            ...
            optimizer = torch.optim.SGD([{'params': self.backbone.parameters()},
...
class S4LScheduler(_LRScheduler):
    ...
    def get_lr(self):
        ...
        if self.last_epoch < self.warmup_until:
            if not self.linear_growth:
                self.linear_growth = [group['lr'] / self.warmup_until for group in self.optimizer.param_groups]
                return self.linear_growth
            else:
                return [group['lr'] + growth for group, growth in zip(self.optimizer.param_groups, self.linear_growth)]
        else:
    ...
    def _get_closed_form_lr(self):
        if self.last_epoch < self.warmup_until:
            return [base_lr * self.last_epoch / warmup_until
